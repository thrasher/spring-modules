...
public class JcrTemplateTests extends TestCase {
	...
	public void testGetImportContentHandler() throws RepositoryException {
		String path = "path";
		MockControl resultMock = MockControl
				.createControl(ContentHandler.class);
		ContentHandler result = (ContentHandler) resultMock.getMock();

		sessionControl.expectAndReturn(
				session.getImportContentHandler(path, 0), result);
		sessionControl.replay();
		sfControl.replay();
	...
	public void testDumpNode() throws RepositoryException {
		...
		MockControl iteratorCtrl = MockControl
				.createControl(PropertyIterator.class);
	...
	public void testGetTree() throws RepositoryException {
		...
		assertSame("Results are not the same", result, tree
				.get("//*/@bogus:title"));

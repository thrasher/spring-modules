 ...
 * execution but also for middle tier transactions via JcrTransactionManager or
 * JtaTransactionManager. In the latter case, Sessions pre-bound by this filter
 * will automatically be used for the transactions.
 * 
 * <p>
 * In contrast to OpenSessionInViewFilter, this interceptor is set up in a
 * Spring application context and can thus take advantage of bean wiring. It
 * derives from JcrAccessor to inherit common JCR configuration properties.
 * 
 * @author Costin Leau
 */
public class OpenSessionInViewInterceptor extends HandlerInterceptorAdapter
	 ...
	 * for the "participate in existing persistence manager handling" request
	 * attribute.
	 * 
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (TransactionSynchronizationManager.hasResource(getSessionFactory())) {
			...
			Integer count = (Integer) request
					.getAttribute(participateAttributeName);
			int newCount = (count != null) ? count.intValue() + 1 : 1;
			request.setAttribute(getParticipateAttributeName(), new Integer(
					newCount));
		}

		else {
			logger.debug("Opening JCR session in OpenSessionInViewInterceptor");
			Session s = SessionFactoryUtils.getSession(getSessionFactory(),
					true);
	...
	public void afterCompletion(HttpServletRequest request,
		...
		if (count != null) {
			...
			if (count.intValue() > 1) {
				request.setAttribute(participateAttributeName, new Integer(
						count.intValue() - 1));
			} else {
			...
			SessionFactoryUtils.releaseSession(sesHolder.getSession(),
					getSessionFactory());
		}
	}
	 ...
	 * representation of the JcrSessionFactory instance and appends ".FILTERED".
	 * 

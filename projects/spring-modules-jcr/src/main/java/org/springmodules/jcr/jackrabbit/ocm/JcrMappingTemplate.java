 ...
 * <p/>
 * For PersistenceManagers the template creates internally the set of default
 * converters.
 * 
 * 
...
public class JcrMappingTemplate extends JcrTemplate implements
	...
	/**
	 * Method for creating a query manager. It's unclear where this entity is
	 * stateless or not.
	 * 
	 * @return
	 */
	public QueryManager createQueryManager() {
		try {
			Map atomicTypeConverters = this.createDefaultConverters(this
					.getSession());
			return new QueryManagerImpl(mapper, atomicTypeConverters, this
	...
	/**
	 * Creates a persistence manager. It's unclear if this object is
	 * stateless/thread-safe or not. However because it depends on session it
	 * has to be created per session and it's not per repository.
	 * 
	 * 
	 * @param session
	 * @return
	 ...
	 */
	protected ObjectContentManager createPersistenceManager(Session session)
			throws RepositoryException, JcrMappingException {
		return new ObjectContentManagerImpl(session, mapper);
	}

	/**
	 * Due to the way the actual jcr-mapping is made we have to create the
	 * converters for each session.
	 * 
	 * @param session
	 * @return
	 */
	protected Map createDefaultConverters(Session session)
	...
	public Object execute(JcrMappingCallback callback)
	 ...
	 * <p>
	 * Note that because we have no base specific exception we have to catch the
	 * generic Exception and translate it into JcrSystemException.
	 * <p>

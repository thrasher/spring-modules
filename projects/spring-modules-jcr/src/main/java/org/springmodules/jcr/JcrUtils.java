 ...
 * based on Brian Moseley <bcm@osafoundation.org> work.
 * 
 * @author Costin Leau
 * 
 */
public abstract class JcrUtils {
	...
	/**
	 * Class used for escaping XML names which contain restricted character as
	 * defined in the JCR spec version 1.0 Section 6.2.5.2. (i.e. "/", ":", "[",
	 * "]", "*", "'", """, "|", and all whitespace characters other than " ").
	 * The escaping schema used is described in the same document in paragraphs
	 * 6.4.3 and 6.4.4.
	 * 
	 ...
	 * 
	 * Implements the encode and decode routines as specified for XML name to
	 * SQL identifier conversion in ISO 9075-14:2003.<br/>
	 * If a character <code>c</code> is not valid at a certain position in an
	 * XML 1.0 NCName it is encoded in the form: '_x' + hexValueOf(c) + '_'
	 * <p/>
	 */
	protected static class ISO9075 {

		/** Pattern on an encoded character */
		private final Pattern ENCODE_PATTERN = Pattern
		 ...
		 * @return the encoded <code>String</code> or <code>name</code> if it
		...
		/**
		 * Encodes the character <code>c</code> as a String in the following
		 * form: <code>"_x" + hex value of c + "_"</code>. Where the hex value
		 * has four digits if the character with possibly leading zeros.
		 * <p/>
		 ...
		 * @param b
		 *            the encoded character is appended to
		 *            <code>StringBuffer</code> <code>b</code>.
		 */
		 ...
		 * character and the following character sequence is 'xHHHH_' where H is
		 * a hex digit.
		 * 
		 * @param name
		 *            the name to check.
